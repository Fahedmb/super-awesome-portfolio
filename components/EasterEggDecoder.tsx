    ...
    <div className="p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="px-3 py-1 rounded-full text-[10px] font-mono font-semibold bg-yellow-400/10 hover:bg-yellow-400/20 text-yellow-300 border border-yellow-400/30 transition-all cursor-pointer flex items-center gap-1.5 active:scale-95"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-3 rounded-xl bg-black/40 border border-white/5 font-mono">
        {/* Cipher Sequence */}
        <div className="flex items-center gap-2">
          {["VI", "I", "VIII", "V", "IV"].map((numeral, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center justify-center w-12 h-12 rounded-lg bg-neutral-900 border border-neutral-700/80 text-neutral-200"
            >
              <span className="text-xs font-bold text-neutral-400">{numeral}</span>
              {isDecoded && (
